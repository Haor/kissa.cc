...
`master` 分支由 Cloudflare Pages 通过 GitHub 自动构建并发布到 `kissa.cc`，
其他分支自动生成 preview 子域。日常发布即 `git push origin master`，本地
不需要跑 wrangler。

如果实在要本地直传 (CF dashboard 挂了 / 紧急回滚)：本地保留有 `wrangler.toml`
（已 gitignore），用 `npx wrangler pages deploy out --project-name=kissa-cc
--branch=master --commit-dirty=true` 直传。
...
4. **首屏 cover "空心" bug**：`document.fonts.ready` 触发的 atlas REBUILD 循环结束时 `TEXTURE0` 绑的是末尾 effect 的 atlas，长时间停留屏（cover）不会再调 `applySlide`，shader 持续读错纹理 → 字符稀疏。修法：REBUILD 完后立刻把 active effect 的 atlas 重 bind 回 `TEXTURE0`（见 `SceneStage.tsx`）。不要回退
5. **Safari className-driven width/height transition 残留**：React 切 class 同时 transition `width`/`height` 时 Safari 会留旧尺寸（`DotNav` 的 active dot 翻页后不复原）。修法：layout 维度只用稳定属性（`opacity` / `transform: translateX`），尺寸不通过 className 切换。当前 `DotNav` 用编号槽 + 滑动下划线（`transform`）规避
...
- `docs/` 是本地工作目录（agent handoff / 调试快照），已 .gitignore，不上 GitHub
